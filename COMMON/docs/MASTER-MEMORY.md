...
- **MONITOR SERVER + ALERT TELEGRAM (5 Sept 2026):** script `/root/SERVER-LINUX/scripts/monitor-server.sh` — tiap 5 menit cek: website localhost:8090 (HTTP 200), cloudflared tunnel, ttyd :7681, node bot.js, RAM ≥90%, disk ≥90%. Kalau ada yang bermasalah → kirim notifikasi ⚠️ ke Telegram OWNER via `TELEGRAM_TOKEN` (dibaca dari `telegram-bot/.env`). Anti-spam pakai state file `.monitor-state` (alert cuma 1x per kejadian, ada "PULIH" saat balik normal). Cron root: `*/5 * * * *`. Log: /var/log/monitor-server.log. Sekarang 3 cron aktif: backup-memory (21:00), monitor (tiap 5 mnt), + auto-backup script
