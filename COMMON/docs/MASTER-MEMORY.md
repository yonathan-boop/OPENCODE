...
- **Terminal online AKTIF di PC Wilianto (22/9):** `methodist-11.my.id/opencode` → ttyd v1.7.7 Windows (exe `%LOCALAPPDATA%\opencode\tools\ttyd\ttyd.exe`) di port 7681, shell PowerShell via `pilih-terminal.ps1` (menu sesi tmux `web-*`), **PROTECTED basic-auth sejak 26/9: username KOSONG + password dari file secrets** (kredensial dibaca dari file secret `C:\Users\WILIANTO\.config\opencode\secrets\ttyd-auth.txt` format `:<password>` — password SENGAJA TIDAK ditulis di repo/memory; log ttyd-autostart.log menyembunyikan password). Restart wajib elevated: proses ttyd jalan admin → `Stop-Process` dari session biasa **gagal "Access is denied"**, harus `Start-Process powershell -Verb RunAs` (UAC) atau lewat task `StartMethodist11Website`. ttyd 1.7.7 **TERVERIFIKASI menerima username kosong** (uji: tanpa kredensial 401, kredensial benar 200 — lokal & publik). Rute: cloudflared → 8090 → `serve8090.js` proxy `/opencode` (HTTP+WebSocket 101) → ttyd 7681. Auto-start: `COMMON\project-sd-methodist-11\start-terminal.ps1` (idempoten, dipanggil dari `start-website.ps1`; arg ttyd sertakan `-t scrollback=20000 -t scrollWithPage=true`). **SCROLL (24/9):** di dalam opencode TUI (fullscreen) scroll mouse TIDAK bisa — pakai **PageUp/PageDown** (user setuju begini saja; lihat LRN-20260924-004). Verifikasi: lokal & publik 200 (setelah auth), WS 101. **Security note:** terminal tetap writable, sekarang dilindungi basic-auth password; untuk lapisan identity-aware (per-user, email OTP) tanpa simpan password di PC → Cloudflare Zero Trust Access (LRN-20260923-003).
...
- **25/9 (yonat-PC, self-learn):** topik baru **8 dimensi Profil Lulusan + SKL resmi Permendikdasmen 10/2025 + Alur Perkembangan Kompetensi SK BSKAP 058/H/KR/2025** → **LRN-20260925-005** (daftar 8 dimensi resmi: keimanan-ketakwaan, kewargaan, penalaran kritis, kreativitas, kolaborasi, kemandirian, kesehatan, komunikasi; deskripsi SKL level SD siap kutip utk KSP bab II; 3 tahapan berkembang/cakap/mahir; **mengoreksi narasi salah di LRN-20260922-004** soal dimensi "literasi digital & numerasi"). Siap dipakai utk KSP bab II Visi-Misi-Tujuan & acuan asesmen karakter.
- **25/9 (yonat-PC, self-learn):** topik baru **struktur kurikulum SD 2026/2027 — alokasi JP per mapel versi resmi Permendikdasmen 13/2025** → **LRN-20260925-004** (rekap JP/minggu per kelas I-VI: 32/34/40/40/42/42; tabel intra per mapel lengkap; koreksi angka generik LRN-002; Bahasa Inggris wajib K3-6 & Koding pilihan K5-6; kokurikuler 1 JP/mapel pengganti P5). Siap dirender jadi tabel struktur kurikulum BAB IV KSP & Lampiran Prota/Promes per mapel.
- **25/9 (yonat-PC, self-learn):** topik baru **perancangan KKTP (Kriteria Ketercapaian Tujuan Pembelajaran)** → **LRN-20260925-003** (KKTP ≠ KKM: deskripsi/rubrik/interval, angka mutlak dilarang; 3 pendekatan resmi, interval umum 0–40/41–65/66–85/86–100 + deskripsi tiap rentang; konversi rubrik→angka Σskor÷(n×maks)×100; format tabel SIMPLE & LENGKAP per modul ajar/PMM; 3–4 kriteria/TP terukur). Siap dipakai di BAB IV Perencanaan Pembelajaran & Lampiran KSP.
- **25/9 (yonat-PC, self-learn):** topik baru **penyusunan perangkat ajar CP→TP→ATP→Modul Ajar utk KSP 2026/2027** → **LRN-20260925-001** (TP = 2 komponen: kompetensi + lingkup materi; 10 prinsip resmi ATP dari Kemendikdasmen — tuntas 1 fase, kolaboratif lintas kelas, SATU alur tak bercabang, fokus CP saja; modul ajar deep learning 7 komponen incl. Pemahaman Bermakna + Pertanyaan Pemantik + Memahami/Mengaplikasi/Merefleksi; CP terbaru = Permendikdasmen 13/2025 + SK BSKAP 046/H/KR/2025 via PMM — jangan pakai CP simpanan tahun lalu; ATP tak ada format baku, sebar ke Prota/Promes pakai minggu efektif riil). Persiapkan lanjutan KSP (Bab IV Perencanaan Pembelajaran & Lampiran).
- **24/9 (yonat-PC, self-learn):** topik baru **konversi PSD/PSB raksasa → JPEG siap cetak** (banner/poster/spanduk desain grafis) → **LRN-20260924-006** (jalur cepat pakai preview flattened via `composite()`; jebakan: Pillow `DecompressionBombError` >178 MP wajib `Image.MAX_IMAGE_PIXELS=None` + psd-tools `PSDLargeImageWarning` >268 MP; SRGBA→RGB + `dpi=(300,300)` utk percetakan; warna desaturasi = ICC `apply_icc=True`; manajemen RAM 2,7GB utk kanvas 47k×14k). Relevan utk banner event sekolah (kasus nyata Banner Retreat ke-13, 24/9).
- **24/9 (yonat-PC, self-learn):** topik baru **pengolahan hasil kuis Google Forms → rekap nilai Excel** → **LRN-20260924-005** (struktur CSV respons: Timestamp/email/kolom pertanyaan; parse skor "7/10"→angka otomatis; deteksi identitas via kolom Nama; predikat KKTP; ambil data via Download .csv atau publish-to-web CSV; Tools & Forms API saat butuh otomasi penuh). Tool teruji: `COMMON/scripts/rekap_forms.py` (`py rekap_forms.py respons.csv [--out rekap.xlsx] [--kktp 90,75,60]`) — output 2 sheet (Rekap + Detail) siap cetak via configure_printing (LRN-003).
...
- **21/9 (yonat-PC):** **Codex CLI terinstall** (v0.155.1 global npm) dengan provider **z0ne.ai** — config `C:\Users\yonat\.codex\config.toml` (base_url `https://api.z0ne.ai/v1`, wire_api `responses`, model `gpt-5.6-sol`, sandbox `unelevated`), env permanen User: `OPENAI_BASE_URL` + `OPENAI_API_KEY` (key z0ne, JANGAN commit ke git). Live test `codex exec` sukses. ⚠️ `npx @z0ne/installer` TIDAK dipakai karena bisa menimpa config OpenCode (model permanen big-pickle).
