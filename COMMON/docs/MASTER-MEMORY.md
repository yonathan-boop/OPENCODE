...
- **ARSIP + HAPUS SESI OPENCODE (11/9):** dari 38 sesi → 32 di-arsip kan (yang terakhir dipakai sebelum 9/9), lalu **dihapus PERMANEN atas permintaan user (part+message+session, DB 54MB→42MB via VACUUM; backup DB sementara di `/tmp/opencode.db.bak`)**. 6 sesi aktif tersisa. Pelajaran: arsip = set `time_archived`, hapus = delete dari tabel `part`→`message`→`session` oleh FK manual.
