...
- **17/9 (server + self-study):** user beri mandat bekerja/belajar mandiri (token murah). Dibangun **SELF-STUDY system** (`COMMON/self-study/` + cron `SERVER-LINUX/scripts/self-study.sh` Senin 03:00) — AI riset web + tulis catatan per topik (topik#1 Dapodik/E-Rapor selesai). **absensi.py** di-upgrade: token-matching (Levenshtein+SequenceMatcher), kasus ambigu → SKIP aman (24/24 kasus lulus). Fix rclone: shared client_id bakal di-retire 2026 → panduan `COMMON/docs/RCLONE-CLIENT-ID.md` (perlu aksi user sekali). Backlog diperbarui.
