...
- **MONITOR SERVER + ALERT (11/9 jadi log-only):** script `/root/SERVER-LINUX/scripts/monitor-server.sh` — cron `*/5 * * * *` cek: website 8090 (HTTP 200), tunnel utama (`cloudflared tunnel run`), ttyd :7681, RAM & disk (alert ≥90%). Alert hanya pertama kali masalah muncul (state file `.monitor-state` anti-spam; pulih hanya di log). Log: /var/log/monitor-server.log. Sebelumnya alert ke Telegram via `@Qksusb_bot` — **dihapus 11/9**.
- **RATE LIMITER WEBSITE (10/9):** serve8090.js sekarang punya rate limiting — **600 request/menit per IP** (sliding window, in-memory Map). IP dideteksi dari header `CF-Connecting-IP` (diambilkan Cloudflare, anti-spoof karena proxied) → fallback X-Forwarded-For → remoteAddress. Kalau lewat → HTTP **429** + header `Retry-After`. Map dibersihkan berkala (>5000 entri → flush entri expired). Test: request 601+ kena 429, IP lain tetap normal, publik 200. Web server di-restart (PID baru) setelah edit. Software restart = cukup `kill <PID>` + `setsid nohup node serve8090.js`; TIDAK perlu ubah start-website.sh (file nama sama).
- **GOOGLE DRIVE BACKUP + AUTO-UPDATE WEBSITE (11/9):** rclone v1.75.1 diinstall (`/usr/local/bin/rclone`), remote `gdrive` (email user, total 5TB / used 49GB). Config: `/root/.config/rclone/rclone.conf` (token OAuth di-generate di yonat-PC via `rclone authorize drive` — **refesh token tidak expired**, bisa di-refresh sendiri). Struktur Drive: `Server Linux Backup/{Memory, SERVER-LINUX, Update website Kegiatan dan Pengumuman}`.
  - **Backup Memory:** rclone copy `/root/memory` → `Server Linux Backup/Memory` (mode copy aman, tidak pernah hapus). ⚠️ lambda? Berat: ribuan file kecil → upload 845 B/s (ETA 1 hari utk 86MB) → plan ganti strategi tar.gz (belum dieksekusi).
  - **AUTO-UPDATE WEBSITE dari Google Drive:** script `/root/SERVER-LINUX/scripts/gdrive-website-check.sh` (di repo SERVER-LINUX) — cron **tiap 15 menit**: pull `Server Linux Backup/Update website Kegiatan dan Pengumuman` → `/root/staging-gdrive` (mode `--ignore-existing`), deteksi file baru via state hash (`/root/staging-gdrive/.processed-files`), kalau ada → `opencode run --auto` di folder website untuk buat/update halaman kegiatan+pengumuman → commit+push → notif (log-only sejak 11/9, Telegram dihapus). Log: `/var/log/gdrive-website-check.log`.
  - **Alur user:** upload file kegiatan/pengumuman terbaru ke `Server Linux Backup/Update website Kegiatan dan Pengumuman` di Google Drive → server otomatis ambil & update website dalam ≤15 menit.
  - **PEMANGKASAN DISK (11/9):** hapus `/opt/google` (Chrome, 447MB), `/opt/noVNC` (20MB), `/usr/lib/snapd` (101MB) → disk `/` turun 87% → 79% (1.6GB avail). Tanpa reboot, tidak ada proses yang butuh itu. **⚠️ NOTICE: apt methods rusak** — `rm -rf /usr/lib/apt/methods` bikin `Stale file handle` pada path itu di overlayfs (dpkg/mkdir/mv semua ESTALE; `remove whiteout` di `/.fly-upper-layer` tidak memulihkan isi lower layer). Efek: `apt-get install` gagal ("method driver http tidak ditemukan"). TIDAK URGENT — semua layanan inti (web 8090, tunnel, ttyd, rclone, cron) tetap jalan tanpa apt. **Sembuh sendiri setelah instance di-reboot** (state overlay direset). Jangan utak-atik /usr/lib/apt/methods lagi sebelum reboot.
...
### TELEGRAM BOT OPENCODE (5 SEPTEMBER 2026 — SERVER LINUX) ❌ DIHAPUS TOTAL 11/9/2026
- **DIHAPUS TOTAL (11/9/2026, permintaan user):** folder `/root/SERVER-LINUX/telegram-bot/` (bot.js, .env yang berisi token, sessions.json, start/stop-bot.sh, workspace) dihapus; `/var/log/telegram-bot.log` & `scripts/notify-telegram.sh` & `scripts/laporan-harian.sh` dihapus. Semua script yang kirim Telegram diubah jadi **log-only** (monitor-server.sh, morning-report.sh, night-shift.sh, gdrive-website-check.sh, health-check.sh). `start-website.sh` [5/5] bot dihapus → sekarang 4 layanan (web 8090, tunnel utama, ttyd, tunnel terminal). Token bot TIDAK pernah masuk git (.env tidak di-track). Sisa riwayat di bawah hanya untuk arsip kalau mau bikin lagi.
- **Fungsi (arsip):** streaming/akses opencode penuh dari HP via Telegram (gantikan terminal web ttyd yang susah dipakai di HP — keyboard HP gak ada Ctrl/tanda panah). Tinggal chat kayak biasa.
- **Bot (arsip):** `@Qksusb_bot` (nama "bot01", token dari BotFather) — **owner-only** (whitelist user id, sekaligus whitelist 5508090479).
- **Folder (arsip):** `/root/SERVER-LINUX/telegram-bot/` (masuk repo SERVER-LINUX recovery kit). Berisi: `bot.js`, `package.json`, `.env` (TOKEN + OWNER_ID + OPCODE_WORKDIR + AUTO_APPROVE=1 + TIMEOUT_MS), `sessions.json`, `start-bot.sh`, `stop-bot.sh`, `workspace/`.
- **Cara kerja (arsip):** tiap pesan chat → `opencode run --continue --dir <workspace>/<uid>/<sesi>` → balasan dikirim/edit di chat. Konteks lanjut per sesi.
...
- **FIXES LAGI (8 Sept 2026):** (1) `log()` sekarang HANYA `appendFileSync` + `start-bot.sh` redirect stdout ke `/dev/null`, stderr ke log → baris log tidak ganda lagi. (2) `TIMEOUT_MS` default **600000** (10 mnt) — gateway free anonymous bisa first-token ~130s saat dipakai ramai; 180s terlalu pendek. **KETAHUAN BARU: `.env` punya `TIMEOUT_MS=120000` yang MENG-OVERRIDE default** — inilah penyebab bot "diam saja" (selalu dibunuh sebelum jawaban tiba); diubah ke 600000 juga di `.env`. (3) Reset sesi bot = pindah `workspace/<uid>/default` → `_archive-<ts>` & bikin default kosong (konteks kecil = balasan lebih cepat). (4) **ACK INSTAN**: tiap pesan langsung dibalas `⏳ Oke, sedang mencari jawaban...` lalu pesan itu DI-EDIT jadi jawaban saat run selesai → tidak ada mode "senyap" walau gateway lambat. (5) **`opencode serve` TIDAK bisa dipakai di server ini**: init menggantung, port tidak pernah bind (dites --pure juga) → tetap pakai spawn `opencode run --continue` per pesan. (6) **Latensi gateway free naik-turun acak** (9s normal, bisa membengkak ke ~3 mnt saat ramai/throttle); `exit=0` tetap tercapai, bukan hang — timeout 600s adalah pengaman, ACK yang bikin UX tetap hidup.
- **AUTO-START (arsip, TIDAK berlaku lagi):** dulu masuk `start-website.sh` langkah [5/5]; sekarang sudah dihapus dari script.
- **Log bot (arsip):** dulu `/var/log/telegram-bot.log`. Start manual: `bash ~/SERVER-LINUX/telegram-bot/start-bot.sh` — SUDAH DIHAPUS.
- **✅ CHECKPOINT STABIL (8 Sept 2026, arsip):** bot pernah dalam kondisi BAIK & andal — semua fix sudah jalan mulus (queue serial, dedupe update_id, timeout 600s, ACK instan). — mengingat status ini arsip karena bot sudah dihapus 11/9.
- **⚠️ KOREKSI ATURAN (8 Sept 2026, arsip):** dulu terminal online & sesi bot boleh berjalan bersamaan; yang WAJIB dipertahankan: proses sesi bot, `node bot.js`, `cloudflared tunnel`. — TIDAK berlaku lagi (bot dihapus).
...
- **Cron aktif (crontab root):**
  ...
  - `10 * * * *` → `health-check.sh` (web 200 local+public, disk/RAM %, proses tunnel/web/ttyd → `/var/log/health.json`)
  - `40 6 * * *` → `morning-report.sh` (laporan pagi: status kesehatan + backlog pending memory → log lokal, Telegram dihapus 11/9)
  - `30 0 * * *` → `night-shift.sh` (opencode dijalankan OTOMATIS tiap malam: pull memory → baca FEATURE-REQUESTS/ERRORS → kerjakan 1-3 tugas aman → commit+push → laporan ke log; `logs/night-shift-<tgl>.md`, timeout 1500s)
- **notify-telegram.sh** (helper kirim Telegram) — **dihapus 11/9**.
- **Pembatasan night-shift:** jangan restart service, jangan sentuh token, jangan hapus permanen, jangan ubah absensi tanpa izin.
- Test: health OK (web/disk/RAM/tunnel), night-shift uji coba dijalankan manual (6 Sept). Cron daemon otomatis dinyalakan start-website.sh setelah reboot.
...
### 11 September 2026 — Server Linux
- **BACKUP GOOGLE DRIVE (strategi tar.gz):** rclone copy per-file dulu sangat lambat (845 B/s, ETA 1 hr untuk 86MB) karena ribuan file kecil. Solusi: `tar -czf` jadi 1 file → upload **13.75 MB/s (6 detik)**. Backup pertama: `backup-memory.tar.gz` (86MB) & `backup-serverlinux.tar.gz` (0.55MB) → sudah upload ke Drive. Script otomatis baru `scripts/backup-gdrive.sh`: tar.gz per tanggal → rclone copy → hapus file lokal; **cron `30 21 * * *`** (habis git auto-backup 21:00). Log: /var/log/gdrive-backup.log.
- **TELEGRAM DIHAPUS TOTAL (11/9):** folder `telegram-bot/`, `notify-telegram.sh`, `laporan-harian.sh`, `/var/log/telegram-bot.log` dihapus. Semua script notif jadi **log-only** (monitor-server, morning-report, night-shift, gdrive-website-check, health-check). `start-website.sh` [5/5] bot dihapus → 4 layanan. Token tidak pernah masuk git. @Qksusb_bot nonaktif; @Methodist-11 (yonat-PC/OpenClaw) masih ada tapi hanya dimatikan (7/9) — hapus total harus di yonat-PC.
- **AUTO-UPDATE WEBSITE TERUJI (11/9):** pipeline Google Drive → opencode berhasil penuh. Uji: upload `TEST-AUTO-UPDATE.txt` ke `Server Linux Backup/Update website Kegiatan dan Pengumuman` → script deteksi (MD5 di `.processed-files`) → opencode buat `pages/pengumuman/uji-coba-auto-update.html` + kartu index + commit `dabfa88` push. Setelah terbukti → file test dihapus dari Drive + commit di-revert (`e261ad6`) → website bersih. Catatan: `opencode run --auto` bisa makan waktu ~10 menit (script cron jalan di background masing2, TIDAK berjejal).
- **PEMANGKASAN DISK (11/9):** hapus `/opt/google` 447MB, `/opt/noVNC` 20MB, `/usr/lib/snapd` 101MB → disk turun 87%→79%. ⚠️ Konsekuensi tak sengaja: `/usr/lib/apt/methods` ikut kena **Stale file handle** → `apt-get install` gagal ("method driver http tidak ditemukan") sampai instance di-reboot (state overlay di-reset). TIDAK urgent — layanan inti (web 8090, tunnel, ttyd, rclone, cron, opencode) tetap jalan tanpa apt. Solusi tanpa reboot untuk install: npm global atau copy binary dari .deb (jangan sentuh /usr/lib/apt/methods).
- **ARSIP + HAPUS SESI OPENCODE (11/9):** dari 38 sesi → 32 di-arsip kan (yang terakhir dipakai sebelum 9/9), lalu **dihapus PERMANEN atas permintaan user (part+message+session, DB 54MB→42MB via VACUUM)**. 6 sesi aktif tersisa. Backup DB (sebelum hapus) di-tar.gz → **disimpan di Google Drive `Server Linux Backup/opencode-db/opencode-db-backup.tar.gz`** (8.3MB, 14/9) lalu dihapus dari server. Pelajaran: arsip = set `time_archived`, hapus = delete dari tabel `part`→`message`→`session` oleh FK manual.
