...
- **RECOVERY KIT:** `linux-server/docs/SERVER-SETUP.md` (langkah pindah server lengkap) + `linux-server/scripts/start-website.sh` (token tunnel ada di variabel TOKEN; juga menyalakan cron daemon)
- **ZEROTIER (24 Agu):** server join network `633e31d8a2212ce2` → IP **192.168.195.60**, node `0f4e41072e`. PC Wilianto = 192.168.195.7 (akses SMB, SSH/RDP closed). WAJIB `chmod 666 /dev/net/tun` sebelum start daemon setelah reboot. Detail: linux-server/docs/ZEROTIER.md
- **AUTO-BACKUP MEMORY HARIAN (22/8):** script `linux-server/scripts/backup-memory.sh` — git pull --rebase → add -A → commit `auto-backup` (skip kalau kosong) → push. Cron root: `0 14 * * *` = **21:00 WIB tiap hari** (server UTC). Log: /var/log/memory-backup.log. Cron daemon dinyalakan otomatis oleh start-website.sh (tidak auto-start setelah reboot, karena tak ada systemd)
