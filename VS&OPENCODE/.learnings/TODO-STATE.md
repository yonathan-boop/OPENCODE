...
## ≡≡≡ BERHASIL ≡≡═���
- **Password terminal online + video no-cache (26/9):** SELESAI — `https://methodist-11.my.id/opencode/` sekarang WAJIB password (basic auth ttyd `-c`), user **dikosongkan** (pilihan user 26/9), password `wm11`. Kredensial dibaca dari file LUAR git: `C:\Users\WILIANTO\.config\opencode\secrets\ttyd-auth.txt` (isi `:wm11`) — JANGAN pernah commit password ini ke repo (pelajaran LRN-20260917-001). Header `WWW-Authenticate` otomatis lolos karena `proxyHttp` di `serve8090.js` meneruskan semua header ttyd. Test: tanpa password 401 · `:wm11` 200 · `admin:wm11` 401 (username HARUS kosong) · `/opencode/token` 200. Swap video jugaaman: `streamVideoFile()` kini menerima param `cacheControl`, route `/video-source/` kirim `no-cache, max-age=0` (sebelumnya `max-age=14400` → Cloudflare edge cache nyajiin video lama 4 jam despite file sudah diganti; Requires query `?cb=` untuk bypass). Commit `cd2fa19`.
- **⚠️ TEMUAN PENTING — Defender Controlled Folder Access memblokir D:\Methodist-11 Document (26/9):** Gejala: "Destination Folder Access Denied" saat Explorer mau hapus/timpa/paste file di share Methodist-11 (D:\Methodist-11 Document + semua subfolder, mis. `#YONATHAN\Video Souce`). Akar: **Windows Defender Controlled Folder Access (anti-ransomware) AKTIF** (`EnableControlledFolderAccess=1`, berubah sendiri 26/9 06:59) + path tersebut masuk daftar protected. Bukti: Event ID 1123 di log `Microsoft-Windows-Windows Defender/Operational` → "powershell.exe has been blocked from modifying D:\Methodist-11 Document\... by Controlled Folder Access". Perilaku: **create file boleh, tapi hapus/rename/timpa DENIED** (ACL folder sudah full `(F)` semua user, jadi BUKAN masalah permission NTFS). Solusi (belum dipakai, user pilih "pindah video dulu"): `Add-MpPreference -ExclusionPath 'D:\Methodist-11 Document'` ATAU `Set-MpPreference -EnableControlledFolderAccess Disabled` (butuh admin). Sisa file tes `__tes.txt` di 3 lokasi (D:\Methodist-11 Document\, #YONATHAN\, #YONATHON\Video Souce\) tidak bisa dihapus karena blokir ini.
